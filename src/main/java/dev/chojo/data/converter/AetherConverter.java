...
public final class AetherConverter {
    public static final ValueConverter<MailSource, String> MAIL_SOURCE = registryConverter(MailSource.class, MailSourceRegistry.INSTANCE);
    public static final ValueConverter<Platform, String> PLATFORM = registryConverter(Platform.class, SupporterRegistry.PLATFORMS);
    public static final ValueConverter<SubscriptionKey, String> SUBSCRIPTION_KEY = registryConverter(SubscriptionKey.class, SupporterRegistry.SUBSCRIPTION_TYPES);
    public static final ValueConverter<PurchaseType, String> PURCHASE_TYPE = registryConverter(PurchaseType.class, SupporterRegistry.PURCHASE_TYPE);
    ...
    private static <V extends Key> ValueConverter<V, String> registryConverter(Class<V> clazz, Registry<V> registry) {
        return ValueConverter.create(
                Adapter.create(
                        clazz,
                        (stmt, index, value) -> stmt.setString(index, value.name()),
                        Types.VARCHAR),
                ValueReader.create(
                        s -> registry.byName(s).orElseThrow(),
                        Row::getString,
                        Row::getString));
    }
}
