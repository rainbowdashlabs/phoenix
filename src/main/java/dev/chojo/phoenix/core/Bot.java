 ...
 */
package dev.chojo.phoenix.core;
...
public class Bot extends AbstractModule {
    ...
    private void jdaCommands(ShardManager manager, Injector injector) {
        ...
        FluavaLocalizer localizer = FluavaLocalizer.create(fluava);
        var error = new ISupporterErrorSupplier() {
            @Override
            public MessageCreateData getCommandError(AccessCheckResult result) {
            ...
            @Override
            public String getAutoCompleteError() {
                return "You do not have permission to use this command";
            }
        };
        var context = new SubcriptionContextProvider() {
            @Override
            public ISubscriptionContext getSubscriptionContext(User user) {
                return Collections::emptySet;
            }

            @Override
            public ISubscriptionContext getSubscriptionContext(Guild guild) {
                return () -> repository.guild(guild.getIdLong()).subscriptions().stream()
                        .map(Subscription::subscriptionId)
        ...
        JDACBuilder builder = JDACommands.builder(manager)
                ...
                .middleware(
                        Priority.HIGH,
                        new SupporterMiddleware<>(context, configuration.main().supporter(), error));
